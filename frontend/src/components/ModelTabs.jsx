      ...
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            className={`flex flex-1 sm:flex-none items-center justify-center sm:justify-start gap-2 whitespace-nowrap px-2 sm:px-5 py-3.5 min-h-[52px] font-headline uppercase text-[0.88rem] sm:text-[0.9rem] tracking-wide sm:tracking-widest border-b-2 -mb-px transition-colors ${
              on
    ...
    <div className="flex flex-wrap gap-x-6 gap-y-2 px-1 py-3 border-b border-[#00FF41]/10">
      {items.map((item) => (
        <div key={item.label} className="flex items-baseline gap-1.5">
          <span className="text-[0.8rem] font-headline uppercase tracking-widest text-[#00FF41]/65">
            {item.label}
          </span>
          <span
